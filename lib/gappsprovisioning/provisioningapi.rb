...
module GAppsProvisioning #:nodoc:
	...
	class GroupEntry
		attr_reader :group_id, :name, :email_permission, :description
		# GroupEntry constructor. Needs a REXML::Element <entry> as parameter
		def initialize(entry) #:nodoc:
			@entry = entry
			entry.elements.each("apps:property") do |e|
				val = e.attributes["value"]
				case e.attributes["name"]
				when "groupId" 					then @group_id = val
				when "groupName" 				then @name = val
				when "emailPermission" 	then @email_permission = val
				when "description" 			then @description = val
				end
	...
	class RequestMessage < Document #:nodoc:
		...
		def about_group(group_id, properties)
			...
			self.elements["atom:entry"].add_element "apps:property", {"name" => "permission", "value" => properties[2] } 
		end
...
# begin
#   user = a.retrieve_user('laura.brink')
#   puts "givenName : "+user.given_name, "familyName : "+user.family_name, "username : "+user.username
#   # puts "admin" ? : user.admin
# rescue EOFError, NoMethodError => e
#   puts "errorcode = "+e.code, "input : "+e.input, "reason : "+e.reason
# end